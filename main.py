...
def ensure_keycrm_offer_sku(offer: dict) -> str:
    ...
    try:
        ...
        # KeyCRM rejects a second variant with an identical property set. This
        # private property makes the SKU-backed analytics variant unique; public
        # catalogue normalization hides every FLW-* variant from customers.
        offer_properties.append(
            {"name": "Website SKU", "value": generated_sku}
        )
        ...
        # Orders are linked to catalogue analytics by SKU. Creating an offer is
        # asynchronous in KeyCRM, so wait until that SKU is actually searchable
        # before the order is sent. Otherwise KeyCRM silently creates a one-time
        # order item which is visible in the order but excluded from Products
        # analytics.
        ...
        app.logger.warning(
            "KeyCRM created offer %s but its SKU is not searchable yet",
            generated_sku,
        )
        return generated_sku
...
def ensure_recent_store_offer_skus():

    """Prepare linked KeyCRM variants used by recent website orders."""
        ...
        offer_ids = sorted(
            {
                ...
                and str(item.get("offer_id") or "").strip().isdigit()
            }
        )

        for offset in range(0, len(offer_ids), 50):

            batch = offer_ids[offset:offset + 50]
            payload = keycrm_request(
                "offers",
                {
                    "limit": 50,
                    "page": 1,
                    "filter[id]": ",".join(batch),
                },
            )

            for offer in payload.get("data", []):

                ensure_keycrm_offer_sku(offer)

    except Exception as error:

        app.logger.warning(
            "Recent website offer SKU preparation failed: %s",
            error,
        )
...
init_db()
ensure_store_invoice_columns()
ensure_recent_store_offer_skus()
...
def ensure_keycrm_offer_sku(offer: dict) -> str:
    ...
    try:
        ...
        # KeyCRM accepts the variant immediately, but its offers search index can
        # lag for a few seconds. Treat the successful create response as the
        # source of truth so the order created in the same checkout is linked by
        # SKU instead of falling back to a one-time analytics-excluded product.
        return generated_sku
